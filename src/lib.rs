...
impl NNTensor {
	pub fn new() -> Result<Self, Box<dyn Error>> {
		let ptr = unsafe { ffi::nn_tensor_init(std::ptr::null::<c_void>() as *mut c_void, std::ptr::null::<ffi::nn_engine>() as *mut ffi::nn_engine) };
		if ptr.is_null() {
	...
	pub fn alloc(&self, ttype: NNTensorType, n_dims: i32, shape: &mut [i32; 3]) -> Result<NNError, NNError> {
		let ttype_c_uint = (ttype as u32) as std::os::raw::c_uint;
		let error = unsafe { ffi::nn_tensor_alloc(self.ptr, ttype_c_uint, n_dims, shape.as_mut_ptr()) };
