...
impl Model {
    ...
    */
    ...
    pub fn layer_datatype_id(&self, index: usize) -> Result<TensorType, Error> {
		...
		if ret == 0 {
			return Err(Error::WrapperError(String::from("Model is invalid or Index is out of range")));
		}
    ...
    pub fn layer_zeros(&self, index: usize) -> Result<&[i32], Error> {
		...
		let ret = unsafe { ffi::nn_model_layer_zeros(self.ptr, index, &mut n_zeros as *mut isize as *mut usize)};
    ...
    pub fn layer_scales(&self, index: usize) -> Result<&[f32], Error>{
		...
		let ret = unsafe { ffi::nn_model_layer_scales(self.ptr, index, &mut n_scales as *mut isize as *mut usize) };
    ...
    pub fn layer_axis(&self, index: usize) -> Result<i32, Error> {
		...
		if ret == 0 { //TODO 0 indicates faliure, but docs say -1 (should be clarified
			return Err(Error::WrapperError(String::from("Index out of range")));
		}
		return Ok(ret);
	}
